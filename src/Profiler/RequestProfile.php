...
namespace ErrorWatch\Sdk\Profiler;
 ...
 *
 * Framework-agnostic: the bag accepts already-snapshotted request/route
 * arrays via {@see startWithSnapshot()}. Each framework adapter
 * (Laravel, Symfony) provides its own request → array converter.
 *
...
class RequestProfile
{
    ...
    /**
     * Start a new profile from a pre-snapshotted request and route.
     *
     * The request snapshot is expected to follow the shape produced by
     * the framework adapter (see Laravel/Symfony Profiler\Snapshot helpers).
     *
     * @param array<string, mixed>|null $requestSnapshot
     * @param array<string, mixed>|null $routeSnapshot
     */
    public function startWithSnapshot(?array $requestSnapshot, ?array $routeSnapshot, ?float $startedAt = null): void
    {
        $this->reset();
        $this->started = true;
    ...
    /**
     * Helper exposed for adapters: filter sensitive headers consistently.
     *
    ...
    /**
     * Helper exposed for adapters: scrub a context array of sensitive keys.
     */
    public static function scrubSensitiveKeys(array $context): array
    {
        ...
        foreach ($context as $key => $value) {
            ...
            $result[$key] = $masked ? '[redacted]' : (is_array($value) ? self::scrubSensitiveKeys($value) : $value);
        }
        return $result;
    }
