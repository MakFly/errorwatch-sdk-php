    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
            ->children()
                ...
                ->arrayNode('user_context')
                    ...
                    ->children()
                        ...
                        ->booleanNode('capture_ip')
                            ->defaultFalse()
                            ->info('Capture IP address in user context (PII under GDPR — opt-in only)')
                        ->end()
                    ->end()
                ->end()
                ->arrayNode('console')
                ...
                ->arrayNode('apm')
                    ...
                    ->children()
                        ...
                        ->arrayNode('cache')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->booleanNode('enabled')->defaultTrue()->info('Emit cache.get / cache.delete spans on the Symfony cache adapter')->end()
                            ->end()
                        ->end()
                ...
                ->arrayNode('profiler')
                    ->addDefaultsIfNotSet()
                    ->info('Capture a full request profile (queries, cache, mail, http, jobs, memory, …) and attach it to every captured exception.')
                    ->children()
                        ->scalarNode('enabled')
                            ->defaultTrue()
                            ->info('On by default. Set "false" or wire %env(bool:ERRORWATCH_PROFILER)% to disable.')
                        ->end()
                    ->end()
                ->end()
                ->arrayNode('logs')
